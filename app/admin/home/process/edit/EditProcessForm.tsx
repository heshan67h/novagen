...
import { updateProcessStep } from '@/app/actions/home';
import { useRouter } from 'next/navigation';
import { useState, useTransition } from 'react';
import { Save, Loader2, Type, ListOrdered } from 'lucide-react';

export default function EditProcessForm({ step, onClose }: { step: any, onClose: () => void }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        startTransition(async () => {
            const res = await updateProcessStep(step.id, formData);
            if (res.success) {
                router.refresh();
                onClose();
            }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="space-y-2">
                    <label className="flex items-center gap-2 text-sm font-medium text-slate-400">
                        <ListOrdered size={16} /> Step Order
                    </label>
                    <input
                        type="number"
                        required
